...
////// Specular:
                ...
                float node_3883 = 0.2;
                float2 node_1837 = float2(((i.uv0.r*node_3883)+(_Score/5.0)),((i.uv0.g*node_3883)+(1.0 - (floor((_Score/5.0))/5.0))));
                float4 _score_alpha_var = tex2D(_score_alpha,TRANSFORM_TEX(node_1837, _score_alpha));
                float3 diffuseColor = lerp((_MainTex_var.rgb*_Color.rgb),_sore_color.rgb,_score_alpha_var.a); // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float2 node_1837 = float2(((i.uv0.r*node_3883)+(_Score/5.0)),((i.uv0.g*node_3883)+(1.0 - (floor((_Score/5.0))/5.0))));
                float4 _score_alpha_var = tex2D(_score_alpha,TRANSFORM_TEX(node_1837, _score_alpha));
                float3 diffColor = lerp((_MainTex_var.rgb*_Color.rgb),_sore_color.rgb,_score_alpha_var.a);
                float specularMonochrome;
                float3 specColor;
                float4 _node_9212_var = tex2D(_node_9212,TRANSFORM_TEX(i.uv0, _node_9212));
