...
////// Specular:
                ...
                float node_5752_ang = (_ScoreRotation*6.28+0.0);
                ...
                float2 node_5752 = (mul(float2(((i.uv0.r*node_3521)+(1.0 - (_Score/5.0))),((i.uv0.g*node_3521)+(floor((_Score/5.0))/5.0)))-node_5752_piv,float2x2( node_5752_cos, -node_5752_sin, node_5752_sin, node_5752_cos))+node_5752_piv);
                float4 _ScoreAlpha_var = tex2D(_ScoreAlpha,TRANSFORM_TEX(node_5752, _ScoreAlpha));
                float3 diffuseColor = lerp(_BaseColor.rgb,_ScoreColor.rgb,_ScoreAlpha_var.a); // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float2 node_5752 = (mul(float2(((i.uv0.r*node_3521)+(1.0 - (_Score/5.0))),((i.uv0.g*node_3521)+(floor((_Score/5.0))/5.0)))-node_5752_piv,float2x2( node_5752_cos, -node_5752_sin, node_5752_sin, node_5752_cos))+node_5752_piv);
                float4 _ScoreAlpha_var = tex2D(_ScoreAlpha,TRANSFORM_TEX(node_5752, _ScoreAlpha));
                float3 diffColor = lerp(_BaseColor.rgb,_ScoreColor.rgb,_ScoreAlpha_var.a);
                float specularMonochrome;
                float3 specColor;
